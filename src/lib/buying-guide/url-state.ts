...
export function catalogHref(answers: Answers, types: Array<'cars' | 'suvs' | 'pickups'>): string {
  ...
  p.set('category', uniq.length === 1 ? TYPE_TO_CATEGORY[uniq[0]] : 'all')
  if (answers.max) p.set('max_price', String(answers.max))
  if (answers.motor) p.set('fuel', FUEL_TO_CATALOG[answers.motor])
  if (answers.prio === 'precio') p.set('sort', 'price_asc')
  else if (answers.prio === 'potencia') p.set('sort', 'power_desc')
  return `/?${p.toString()}`
}
